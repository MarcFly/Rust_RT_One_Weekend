...
pub struct hittable_list {
    pub obj_list: Vec<Arc<Box<dyn Hittable>>>,
    bvh_start: bvh_node,
    pub num_nodes: i32,
}
...
impl hittable_list {
    ...
    pub fn hit_debug(&self, t_min: f64, t_max: f64, rec: &mut hit_record, r: &ray) -> bool {
        let mut temp_rec = hit_record::new();
        let mut hit_anything = false;
        let mut closest = t_max;
        ...
        for obj in &self.obj_list {
            rec.iters += 1;
            if(obj.hit_debug(r, t_min, t_max, &mut temp_rec) && temp_rec.t < closest) {
                hit_anything = true;
                closest = temp_rec.t;
                *rec = temp_rec.clone();
            }
    ...
    pub fn construct_bvh(&mut self, time0: f64, time1: f64) {
        self.bvh_start = bvh_node::new(&mut self.obj_list[..], time0, time1, &mut self.num_nodes);
    }

    pub fn copy_self(&self) -> hittable_list {
        hittable_list { 
            obj_list: self.obj_list.clone() ,
            bvh_start: bvh_node::new_empty(),
            num_nodes: 0,
         }
    }

    pub fn debug_bvh_aabb(&self, time0: f64, time1: f64) -> hittable_list {
        let mut ret_list = hittable_list::new();
        let mut arc_list = Arc::new(Mutex::new(ret_list));
        self.bvh_start.fill_list(&mut arc_list);


        let cpy = arc_list.lock().unwrap().copy_self();
...
use crate::materials::*;
struct bvh_node {
    left_ch: Arc<Box<dyn Hittable>>,
    right_ch: Arc<Box<dyn Hittable>>,
    pub aabb_box: aabb,
...
impl bvh_node {
    pub fn new_empty() -> bvh_node {
        bvh_node { 
            left_ch: Arc::new(Box::new(aabb::new())), 
            right_ch: Arc::new(Box::new(aabb::new())), 
            aabb_box: aabb::new(),
    ...
    pub fn new(obj_list: &mut [Arc<Box<dyn Hittable>>], time0: f64, time1: f64, num_nodes: &mut i32) -> bvh_node {
        ...
        if list_len == 1 {
            ...
            if obj_list[0].compare(obj_list[1].as_ref(), axis) {
                ret_node.left_ch = Arc::clone(&obj_list[0]);
            ...
            let mid = list_len / 2;
            ret_node.left_ch = Arc::new(Box::new(bvh_node::new(&mut obj_list[0..mid], time0, time1, num_nodes)));
            ret_node.right_ch = Arc::new(Box::new(bvh_node::new(&mut obj_list[mid..], time0, time1, num_nodes)));
        }
...
impl Hittable for bvh_node {
    ...
    fn fill_list(&self, list: &mut Arc<Mutex<hittable_list>>) {
        list.lock().unwrap().obj_list.push(Arc::new(Box::new(aabb::from(self.aabb_box.min, self.aabb_box.max))));

        self.left_ch.fill_list(list);
        self.right_ch.fill_list(list);
