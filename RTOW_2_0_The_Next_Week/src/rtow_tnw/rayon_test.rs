...
//
        ...
        let par_iter = cols.into_par_iter().map(|mut pixel| {
            //let hit_arc = Arc::clone(&arc_hit);
            //let light_arc = Arc::clone(&arc_lights);
            //let curr_pixel = Arc::clone(&pixel);
            //let clone_iters = Arc::clone(&arc_iters);
            ...
            for s in (0..samples) {
                ...
                let r = cam.focus_time_ray(u, v);
                let ambient_indirect = ray_hits(&r, Arc::clone(&arc_hit), depth, Arc::clone(&arc_iters));
                //let lights_direct = light_hits(&r, Arc::clone(&light_arc), Arc::clone(&hit_arc));
                pixel.color = pixel.color + ambient_indirect;// + lights_direct;
            }                   
            //pixel.write_color(curr_pixel, 0);
