...
pub fn output_cam_focus() {
    ...
    let mut cols: Box<Vec<colorRGB>> = Box::new(Vec::new());
    ...
    for i in (0..(image_height)).rev() {
        ...
        for j in (0..image_width) {
            ...
            let idx = (image_width * (image_height - i - 1 ) as usize + j );
            ...
            for s in (0..samples) {
                ...
                cols[idx] = cols[idx] + ray_hits(&r, &hittables, depth);
            }
            //pixel.write_color(samples as f64);
        }
    }
    eprintln!("\nDone with {} samples per pixel", samples);
    eprintln!("Tracing took {} ms", timer.ms());


    for p in *cols {
        p.write_color(samples as f64);
    }

    eprintln!("Then std::out overhead {} ms", timer.ms());
}