...
pub fn render() {
...

    {
        ...
        let mut max_phys_mem_og = Arc::new(Mutex::new(0 as i32));
        let mut min_phys_mem_og = Arc::new(Mutex::new(f32::INFINITY as i32));
        let mut max_virt_mem_og = Arc::new(Mutex::new(0 as i32));
        let mut min_virt_mem_og = Arc::new(Mutex::new(f32::INFINITY as i32));
        
        if let Some(usage) = memory_stats() {
            eprintln!("Recursive PreLoop Physical Mem: {}", usage.physical_mem / 1024 / 1024);
            eprintln!("Recursive PreLoop Virtual Mem: {}", usage.virtual_mem / 1024 / 1024);
        };
        eprintln!();
    ...
    if let Some(usage) = memory_stats() {
        eprintln!("Recursive PostLoop Physical Mem: {}", usage.physical_mem / 1024 / 1024);
        eprintln!("Recursive PostLoop Virtual Mem: {}", usage.virtual_mem / 1024 / 1024);
    };
    eprintln!();
