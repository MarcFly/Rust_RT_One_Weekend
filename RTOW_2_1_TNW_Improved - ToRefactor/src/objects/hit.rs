...
pub trait Hittable: Send + Sync {
    ...
    fn compare(&self, other: &Arc<Objects>, axis: usize) -> bool {
        let (check, box1) = self.get_aabb(0., 0.);
        let (check2, box2) = object_match_aabb_arc(other, 0., 0.);
...
pub fn compare_x(main: &Arc<Objects>, other: &Arc<Objects>) -> std::cmp::Ordering {
    let (check, box1) = object_match_aabb_arc(main, 0., 0.);
    let (check2, box2) = object_match_aabb_arc(other, 0., 0.);
...
pub fn compare_y(main: &Arc<Objects>, other: &Arc<Objects>) -> std::cmp::Ordering {
    let (check, box1) = object_match_aabb_arc(main, 0., 0.);
    let (check2, box2) = object_match_aabb_arc(other, 0., 0.);
    ...
    if !check || !check2 { 
        let (check, box1) = object_match_aabb_arc(main, 0., 0.);
        let (check2, box2) = object_match_aabb_arc(other, 0., 0.);
        panic!("Passed value without aabb!!!!")
    };
...
pub fn compare_z(main: &Arc<Objects>, other: &Arc<Objects>) -> std::cmp::Ordering {
    let (check, box1) = object_match_aabb_arc(main, 0., 0.);
    let (check2, box2) = object_match_aabb_arc(other, 0., 0.);
